...
During our analysis we used the following R and Biocunductor packages
\Begin{table}[htb]
\caption{List of all packages used in R for obtaining, processing and analysis of data.}
\resizebox{\textwidth}{!}{
\begin{tabular}{lllll}
\hline
affy & vsn & AnnotationDbi \\
hgu133plus2hsenstcdf & hgu133plus2hsenstprobe & limma\\
pheatmap & dplyr & tidyverse \\
hexbin & cluster & factoextra\\
EnhancedVolcano & org.Hs.eg.db & RColorBrewer\\
VennDiagram & GO.db \\
\hline
\end{tabular}}
\end{table}
Over the course of the analysis an array of packages was used which are listed in Table 1. 
...
```{r normalization & meanSdPlot, echo=FALSE, message=FALSE, comment=FALSE, results="hide", highlight=TRUE}
#Normalization
...
# fusion of fusion.tra.expression.tra.table and ensembl.only.tra (aware that ensembl.only.tra table contains less rows than the other two tables due to the 160 missing genes)
...
names(tissue.distribution)=c("max.tissue","tissue.number")
...
barplot(height = as.numeric(tissue.distribution.arranged$tissue.number[1:10]),names.arg = tissue.distribution.arranged$max.tissue[1:10],cex.names=0.5,col=coul, main="Frequency of TRAs in our data", ylab="Frequency")
...
plot(pca$x[,1], pca$x[,2],col=color1, pch=19,xlab='PC1',ylab='PC2')
...
#Plot PC2 and PC3
plot(pca$x[,2], pca$x[,3],col=color1, pch=19,xlab='PC2',ylab='PC3')

#Plot PC3 and PC4
plot(pca$x[,3], pca$x[,4],col=color1, pch=19,xlab='PC3',ylab='PC4')
```
...
ggplot(sil_df, aes(x = k, y = sil_width)) +
  geom_line() +
  scale_x_continuous(breaks = 2:10) + labs(y= "Silhouette Width")
```
...
p1 <- ggplot(annotated.limma.table.vollst.1.8, aes(logFC, -log(adj.P.Val,10)))+
  ...
  ggtitle("Volcanoplot between 1-cell stage and 8-cell stage")+
...
```{r}
#group the genes into "Up-regulated","Down-regulated" and "Unchanged" for colouring volcanoplot
...
#create a volcanoplot
p1 <- ggplot(annotated.limma.table.vollst.8.m, aes(logFC, -log(adj.P.Val,10)))+
  geom_point(aes(color = Expression),size = 2/5)+  
  ggtitle("Volcanoplot 8-cell stage between morula stage")+
...
#create a volcanoplot
p1 <- ggplot(annotated.limma.table.vollst.m.b, aes(logFC, -log(adj.P.Val,10)))+
  geom_point(aes(color = Expression),size = 2/5)+  
  ggtitle("Volcanoplot morula stage between blastocyst stage")+
  theme(plot.title = element_text(hjust = 0.5,face = "bold",size=22))+
  xlab(expression("log"[10]*"FC")) + 
  ylab(expression("-log"[10]*"(adj.P.Val)"))+
  geom_vline(xintercept=c(-2, 2), col="black",linetype = "dashed") +
...
enrich_go = kegga(fit.8.m, geneid = annotated.human.vsnrma.df2$entrez, species = "Hs")
  a=topKEGG(enrich_go, ontology ="BP")
...
#gene.pathway als Argument oder zb getGeneKEGGLinks(species.KEGG).

a=SEA.function.GO(fit.8.m)
b=SEA.function.GO(fit.m.b)
```
...
display_venn.up <- function(x){
  grid.newpage()
  venn_object <- venn.diagram(x, main=("upregulated TRAS between diffrent stages"), category.names=c("1-cell and 8-cell stage"," 8-cell and morula stage","morula and blastocyst stage"), fill = c("#009E73", "#E69F00", "#56B4E9"), filename= NULL)
...
#extract chemokine
chemokine.tra.list=fusion.tra.expression.tra.table.ensembl.table[which(grepl("motif chemokine",fusion.tra.expression.tra.table.ensembl.table[,24])),]
