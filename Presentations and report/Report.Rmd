...
### 3.1.2 TRA dataset
As a reference TRA dataset we chose the GTEX RNAseq human dataset provided by Dr. Dinkelacker in 2019. It consisted of 60131 transcripts. We used the TRA dataset and ensembl gene information to annotate our raw data.
...
hgu133plus2hsenstprobe 25.0.0 & pheatmap 1.0.12 & EnhancedVolcano 1.14.0 & tinytex 0.40 & RColorBrewer 1.1-3 \\
clusterProfiler 4.4.4 & enrichplot 1.16.1 & imager 0.42.13\\
\hline
\end{tabular}}
\end{table}
...
```{r only chip GSM456645,  include=TRUE, fig.cap="Single chip image of chip GSM456645",fig.height=4, fig.width=3.6,fig.align="center"}
...
```{r meanSdPlot, include=TRUE, fig.cap="MeanSd plot", fig.height=3,fig.width=4.5,fig.align='center'}
...
We wanted to investigate our hypothesis whether or not there are even differential expressed TRAs in our dataset by visualizing the distribution of the TRAs differential expressed in our chips according to their max tissue, using a boxplot.
...
(Their function kurz erwähnen? Clara?)
...
```{r GSEA for stage m-b,include=TRUE, fig.cap="Gene set enrichment analysis comparing morula and blastocyst stage"}
...
require(DOSE)
dotplot(gse.m.b.DEG, showCategory=20, split=".sign",font.size=3) + facet_grid(.~.sign)
#gse.m.b.DEG@result
```
...
heatmap=pheatmap(as.matrix(chemokine.expression[,1:18]), cluster_cols=FALSE,cluster_rows=TRUE, show_rownames = TRUE, legend=TRUE, fontsize_row=0.5, annotation_row = annotation_row)
...
knitr::kable(top_genes_tra.8.m[1:10,f],format = "simple",caption ="Updownregulated genes comparing 8-cell and morula stage")
