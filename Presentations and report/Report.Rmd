...
```{r only chip GSM456645,  include=TRUE, fig.cap="Single chip image of chip GSM456645",fig.height=3.6, fig.width=3.2,fig.align="center"}
#image(data.human[,3], col = rainbow (100, start = 0, end = 0.75)[100:1],main = substr(colnames(data.human)[3],1,9))

img<-image_read('chip_GSM456645.png') 
...
```{r meanSdPlot, include=TRUE, fig.cap="MeanSd plot", fig.height=2,fig.width=4.5,fig.align='center'}
...
```{r Tissue distribution plot, include=TRUE, fig.cap="TRA tissue distribution in our data", fig.height=3.2, fig.width=7, fig.align="center"}
...
It seems like even though there is no tissue development in the observed stages, certain TRAs are still differentially expressed. We also analyzed the original TRA dataset provided by Dr. Dinkelacker (2019) to substantiate the validity of this distribution. We noted a high level of testis genes in the original data.
...
```{r Venn diagram up, include=TRUE, fig.cap="Venn diagram of up-regulated genes", fig.height=3, fig.width=4.5, fig.align='center'}
...
display_venn.up = function(x){
  grid.newpage()
  venn_object = venn.diagram(x, main=("Up-regulated TRAS between different stages"), category.names=c("1-cell and 8-cell stage"," 8-cell and morula stage","morula and blastocyst stage"), fill = c("#009E73", "#E69F00", "#56B4E9"), filename= NULL,cat.cex=0.6)
...
```{r Venn diagram down, fig.cap="Venn diagram down-regulated genes", fig.height=4, fig.width=4, fig.align="center"}
...
display_venn.down = function(x){
  grid.newpage()
  venn_object.down = venn.diagram(x, main=("Down-regulated TRAS between different stages"), cex.main=2, font.main=1, col.main="red", category.names=c("1-cell and 8-cell stage"," 8-cell and morula stage","morula and blastocyst stage"),fill = c("#009E73", "#E69F00", "#56B4E9"), filename=NULL)
  grid.draw(venn_object.down)
}

# Create Venn diagramm with down-regulated genes
...
```{r chemokine heatmap,include=TRUE,fig.cap="Differentiall expression of different chemokines",fig.height=3, fig.width=5}
...
## 6.1 Top 10 up-regualted TRA genes in the compared stages
...
```{r Tissue distribution plot of tra.data,include=TRUE, fig.cap="TRA tissue distribution in our data", fig.height=3.2, fig.width=7, fig.align='center'}
...
par(mar=c(5,9,4,1)+.1)
barplot(height = as.numeric(tissue.distribution.arranged$tissue.number[1:10]),names.arg = tissue.distribution.arranged$max.tissue[1:10],cex.names=0.54,col=rainbow(10), main="Frequency of TRA genes in tra.data", cex.main= 0.9, xlab="Frequency",las=2, horiz= TRUE, cex.lab=1, cex.axis= 1,xlim=c(0,25000))
